...
### Post-Deploy: Initialize Elasticsearch Indices

Run these once after the initial deploy (and after any ES cluster reset):

```bash
POD=$(kubectl get pod -n codepathfinder -l app=codepathfinder \
  -o jsonpath="{.items[0].metadata.name}")

# Memories knowledge base index (ELSER semantic search)
kubectl exec $POD -n codepathfinder -c web -- \
  python manage.py create_memories_index

# Ask CodePathfinder chat history index
kubectl exec $POD -n codepathfinder -c web -- \
  python manage.py create_chat_sessions_index
```

Both commands are idempotent — safe to re-run. If mappings drift (e.g. after a schema change), pass `--recreate` to drop and rebuild the index:

```bash
kubectl exec $POD -n codepathfinder -c web -- \
  python manage.py create_chat_sessions_index --recreate
```

> **Elastic Cloud Serverless note**: the `create_chat_sessions_index` command deliberately omits `number_of_shards` / `number_of_replicas` settings, which are rejected by Serverless. This is intentional — do not add them back.
...
#### 5. Ask CodePathfinder Chat History Not Saving
*   **Cause**: The `panel_chat` Elasticsearch index is missing or has a stale mapping.
*   **Check**:
    ```bash
    POD=$(kubectl get pod -n codepathfinder -l app=codepathfinder \
      -o jsonpath="{.items[0].metadata.name}")
    kubectl exec $POD -n codepathfinder -c web -- \
      python manage.py create_chat_sessions_index
    ```
    If it reports a mapping conflict (e.g. `mapper [role] cannot be changed from type [text] to [keyword]`), recreate the index:
    ```bash
    kubectl exec $POD -n codepathfinder -c web -- \
      python manage.py create_chat_sessions_index --recreate
    ```
*   **Note**: `--recreate` deletes existing chat history. Only use it when history is already broken/empty.

#### 6. Console Not Showing Logs
...
3. **Rebuild and Deploy**:
   ...
   ```bash
   # Build & Push
   docker build --platform linux/amd64 -f Dockerfile.prod -t <YOUR_REGISTRY>/codepathfinder/web:vX.Y.Z .
   docker push <YOUR_REGISTRY>/codepathfinder/web:vX.Y.Z

   # Update Deployment
   kubectl set image deployment/codepathfinder web=<YOUR_REGISTRY>/codepathfinder/web:vX.Y.Z -n codepathfinder
   ```
4. **Client Refresh**:
