...
Use `github_manage_code` (action="get_info") and `semantic_code_search` to discover:
...
|------|---------|---------------|
| `github_manage_code` (action="get_info") | Repository metadata | Get name, description, default branch |
...
| Tool | Purpose | Example |
|------|---------|---------|
| `github_manage_issues` (action="create_issue") | Documentation tickets | Create improvement issues |
