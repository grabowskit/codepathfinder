...
class JobStartView(APIView):
    ...
    def post(self, request, project_id):
        ...
        # Clear progress fields from any previous run
...
class JobStatusView(APIView):
    ...
    def get(self, request, project_id):
        ...
        # Build status text
        stage_label = project.current_stage or project.status
        status_text = f"Project '{project.name}' is {project.status}"
        if project.status == 'running':
            if project.current_stage:
                status_text += f" ({project.current_stage}"
                if project.progress_pct:
                    status_text += f", ~{project.progress_pct}%"
                status_text += ")"
            elif es_stats:
        ...
        data = {
            ...
            'progress': {
                'stage': project.current_stage or None,
                'progress_pct': project.progress_pct,
                'total_files': project.total_files,
                'files_processed': project.files_processed,
                'stage_message': project.stage_message or None,
                'last_progress_at': project.last_progress_at.isoformat() if project.last_progress_at else None,
            },
            'options': {
                'clean_index': project.clean_index,
                'pull_before_index': project.pull_before_index,
...
class JobResetView(APIView):
    ...
    def post(self, request, project_id):
        ...
        # Reset to pending and clear progress
        project.status = 'pending'
        project.save()
        reset_project_progress(project)
