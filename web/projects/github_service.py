...
class GitHubService:
    ...
    # Latest Changes Operations

    def get_recent_commits(self, branch=None, limit=10):
        """
        Get recent commits for the repository.

        Args:
            branch: Optional branch name to fetch commits from (defaults to default branch)
        ...
        try:
            ...
            raise GitHubPermissionError(f"Failed to get recent commits: {e.data.get('message', str(e))}")
