...
class PathfinderProject(models.Model):
    ...
    # Indexing progress tracking (Phase 1: derived from K8s pod logs + ES doc count)
    current_stage = models.CharField(max_length=20, blank=True, default='', help_text="Current indexing stage (initializing/cloning/enqueuing/processing/finalizing/watching/stalled/error/stopped)")
    progress_pct = models.IntegerField(default=0, help_text="Estimated percent complete (0-100); monotonically increasing within a run")
    total_files = models.IntegerField(default=0, help_text="Total files to index, parsed from 'Found N files to process' log line")
    files_processed = models.IntegerField(default=0, help_text="Proxy: current ES document count divided by avg chunks/file")
